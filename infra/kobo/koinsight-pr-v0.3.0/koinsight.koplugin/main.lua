...
-- Debounce window for NetworkConnected-triggered syncs. Prevents thrashing
-- on rapid connect cycles (e.g. quick sleep/wake or network flapping).
local NETWORK_CONNECTED_SYNC_DEBOUNCE_S = 30
...
function koinsight:addToMainMenu(menu_items)
  menu_items.koinsight = {
    ...
    sub_item_table = {
      ...
      -- 5) Sync when Wi-Fi connects (recommended for Kobo)
      -- On Kobo, wifi is always killed before plugin onSuspend runs, so
      -- sync-on-suspend requires aggressive mode which blocks the suspend
      -- for ~10s while re-connecting. Syncing on NetworkConnected instead
      -- (e.g. after auto_restore_wifi on resume) is silent and non-blocking.
      {
        text = _("Sync when Wi-Fi connects (recommended for Kobo)"),
        checked_func = function()
          return self.koinsight_settings:getSyncOnNetworkConnectedEnabled()
        end,
        callback = function()
          self.koinsight_settings:toggleSyncOnNetworkConnected()
        end,
      },
...
-- Fires after wifi finishes connecting (e.g. silent auto_restore_wifi on resume,
-- or any manual/menu-driven wifi enable). Non-blocking background sync.
function koinsight:onNetworkConnected()
  if not self.koinsight_settings:getSyncOnNetworkConnectedEnabled() then
    return
  end

  local now = os.time()
  if self._last_sync_attempt and (now - self._last_sync_attempt) < NETWORK_CONNECTED_SYNC_DEBOUNCE_S then
    logger.dbg("[KoInsight] NetworkConnected: within debounce window, skipping")
    return
  end

  if self._sync_in_progress then
    logger.dbg("[KoInsight] NetworkConnected: sync already in progress, skipping")
    return
  end

  local server_url = self.koinsight_settings:getServerURL()
  if not server_url or server_url == "" then
    logger.dbg("[KoInsight] NetworkConnected: no server URL, skipping")
    return
  end
  ...
  logger.info("[KoInsight] NetworkConnected - scheduling background sync")

  -- Defer by one tick so we don't block the event dispatch itself.
  UIManager:scheduleIn(1, function()
    local ok, err = pcall(function()
      KoInsightUpload.syncCurrentBook(server_url, true) -- silent mode
    end)
    self._sync_in_progress = false
    if not ok then
      logger.warn("[KoInsight] NetworkConnected sync failed:", tostring(err))
    else
      logger.info("[KoInsight] NetworkConnected sync completed")
    end
  end)
end
