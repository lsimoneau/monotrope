...
function KoInsightSettings:getSyncOnNetworkConnectedEnabled()
  local value = self.data.sync_on_network_connected
  if value == nil then
    return DEFAULTS.sync_on_network_connected
  end
  return value
end
function KoInsightSettings:setSyncOnNetworkConnectedEnabled(enabled)
  return self:update({ sync_on_network_connected = (enabled == true) })
end
function KoInsightSettings:toggleSyncOnNetworkConnected()
  local current = self:getSyncOnNetworkConnectedEnabled()
  local new_value = not current
  local success = self:setSyncOnNetworkConnectedEnabled(new_value)

  if success then
    local message = new_value and _("Sync when Wi-Fi connects enabled")
      or _("Sync when Wi-Fi connects disabled")
    UIManager:show(InfoMessage:new({ text = message, timeout = 2 }))
    logger.info("[KoInsight] Sync on network connected toggled from", current, "to", new_value)
  else
    UIManager:show(
      InfoMessage:new({ text = _("Error toggling network-connected sync setting"), timeout = 3 })
    )
    logger.err("[KoInsight] Failed to toggle sync_on_network_connected")
  end
